...
async def fetch_repo_index(
    ...
    for item in tree_raw.get("tree", []):
        tree_items.append(
            TreeItem(path=item["path"], type=item["type"], size=item.get("size"))
        )
    readme = None
    try:
    ...
    if "license" in repo_meta_raw and repo_meta_raw["license"]:
        license_name = repo_meta_raw["license"].get("spdx_id") or repo_meta_raw[
            "license"
        ].get("name")
    meta = RepoMetadata(
    ...
    return RepoIndexResponse(
        repo=meta, tree=tree_items, branches=branches, readme=readme
    )
...
async def fetch_commits(
    owner: str, repo: str, ref: Optional[str] = None, per_page: int = 20
) -> CommitsResponse:
...
async def search_code(
    owner: str, repo: str, query: str, per_page: int = 20
) -> SearchCodeResponse:
